...
| USGS Cooperative National Geologic Map (v2) | Overlay + identify | Public ArcGIS vector tiles `Hosted/mapunitpolys_{esurf\|quat\|prequat\|precamb}_v2` and point-identify FeatureServers `Hosted/mapunitpolys_*_labels/FeatureServer/0` (Earth Surface default: `https://energy.usgs.gov/arcgis/rest/services/Hosted/mapunitpolys_esurf_v2/VectorTileServer/tile/{z}/{y}/{x}.pbf`). Themes match the [National Geology viewer](https://ngmdb.usgs.gov/nationalgeology/) (Earth Surface / Quaternary / Pre-Quaternary / Precambrian). Symbology: National Synthesis, GeoMaterial, Age, Source Geology (client-side styling of the same tiles). Identify fields include map unit name/code, synthesis description, GeMS GeoMaterial, age, source-map citation, NGMDB URL. | USGS / AASG / National Cooperative Geologic Mapping Program — public domain. Product: [DR-1210](https://ngmdb.usgs.gov/Prodesc/proddesc_118545.htm); Earth’s surface data [doi:10.5066/P146VGVM](https://doi.org/10.5066/P146VGVM). **Not a replacement for SGMC** — keep both toggles. **Not a collecting permit.** Synthesis scale ≠ outcrop; verify claims/BLM/private land. Tiles + identify require **Settings → Online Enrichment**. |
...
**Prospect Guess** (Map tap or the Prospect Guess button) queries the CNGM FeatureServer at the point (when Online Enrichment is on), shows the unit card, and ranks gem/mineral hints from GeoMaterial keywords plus nearby curated/USGS localities. Heuristics live in `GeoMaterialMineralCatalog` / `knowledge/geology/geomaterial-mineral-associations.md`. Results are probabilistic research aids with Stronger / Plausible / Speculative bands — never certainty or access permission. Identify responses are cached in memory for a few minutes and fail soft when offline.
